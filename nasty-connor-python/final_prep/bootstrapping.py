...
def print_result(data):
    # Header
    print(f"{'Time':>5} | {'Interest':>11}")
    print("-" * 19)

    # Data rows
    for time, interest in data:
        print(f"{time:5.2f} | {interest:11.8f}")

def price_guess(guess_r, cpn_total_payout, cpn_payment, cpn_freq, OFFSET, face, bond, result, result_correct=False):
...
def price_deriv(guess_r, cpn_payment, bond, cpn_freq, OFFSET, face, result):
    time, _, _, _ = bond
    last_bond_time, last_bond_yield = result[-1]
    cpn_time = last_bond_time + cpn_freq + OFFSET
...
def bootstrap_zerorate(initial_r, face, cpn_freq, result, bonds, OFFSET, newton_output):
    ...
    for bond in bonds:
        ...
        while abs(guess_old - guess_r) > tol_approx:
            guess_old = guess_r
            num = price_guess(guess_r, cpn_total_payout, cpn_payment, cpn_freq, OFFSET, face, bond, result)
            denom = price_deriv(guess_r, cpn_payment, bond, cpn_freq, OFFSET, face, result)
            sub = num / denom
            guess_r = guess_r - sub
            if bond[3]:
        ...
        price_guess(guess_r, cpn_total_payout, cpn_payment, cpn_freq, OFFSET, face, bond, result, result_correct=True)
